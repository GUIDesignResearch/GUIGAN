# -*- coding: utf-8 -*-
"""
Created on Sun Jul 21 12:56:57 2019
(7)做subtree pairs 来train net
@author: ztm
"""
...
def write_pair(_pair_file, train_intra_pair, train_inter_pair): # 把随机挑选的UI对存入txt文件，用；分开intra和inter的pair
...
def read_pair(_pair_file): # 读取UI的pair，分号；用来分开intra和inter
...
def get_soft_all_image_pair(dictionary, available_apps,ui_c=20,s_num=10): # 取类内pair和类间pair数据
    _count = 0
    intra_pair = []
    available_apps1 = [] # 有效app
...
#        print('available_uis_nums: ', len(current_app_dirs))
        ...
        if len(arr)>=ui_c:
            ...
            _count+=1
            arr_c = arr[:ui_c] # 每个app随机取10个subtree
#            print('random select: ', arr) # 随机取类内的一对图像
            
            m = int(len(arr_c)/2)
            for j in range(m):
#                print('Ui pair: ',arr_c[j], arr_c[j+m])
    ...
    #    inter_comb = list(combinations(inter, 2)) # 所有app的排列组合，太多了1241100个
    #    print(inter_comb)
    inter_pair = []
